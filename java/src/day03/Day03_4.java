...
public class Day03_4 {
	...
	//키오스크 프로그램
		//판메제품: 1. 콜라[500] 2. 커피[600] 3. 스무디[700] 4.결제
			//선택한 제품을 장바구니에 담기
				//결제 여부 물어보고 현금 입력받기 [ 잔돈 출력후 종료 ]
		//제품재고: 콜라[10게] 커피[10개] 스무디[10개]
			//재고가 없을경우 재고없은 출력
		...
		while(true) {//outer
			...
			//변수 선언 / 장바구니, 재고
				//지역변수: { } 안에서 선언된 변수는 { } 밖으로 못나감
					//{ } 밖에서 선언된 변수는 { } 안으로 사용 가능
			int 콜라구매수 = 0;
			int 커피구매수 = 0;
			int 스무디구매수 = 0;
			int 총결제액 = 0;
			
			while(true) { //무한루프 inner while starts
				...
				int 선택 = scan.nextInt(); //선택받기
				if( 선택 == 1 ) {
					if( 콜라재고 == 0 ) {
						System.out.println( "[준비중] 재고가 없습니다" );
					}else {
				...
				if( 선택 == 3 ) {
					if( 스무디재고 == 0 ) {
						...
						System.out.println( "스무디를 담았습니다");
						스무디구매수++;
					}
				}
				if( 선택 == 4 ) {
					System.out.println( "위 장바구니에 제품을 결제 하시겠습니까? [ y/n ]");
					String 결제선택 = scan.next();
